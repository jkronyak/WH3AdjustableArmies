...
local function handle_selected_army(context)
    ...
    out(cm:are_pooled_resources_tracked_for_faction(faction:name()))
        ...
        if og_unit.purchased_effects:num_items() > 0 then
            ...
            for j=0, og_unit.purchased_effects:num_items() - 1 do
                ...
                if unit_purchasable_effect_mapping_grn[cur_key] then
                    ...
                    local spent_b, gained_b = cm:get_total_pooled_resource_changed_for_faction(faction:name(), resource, factor)
                    ...
                    local spent_a, gained_a = cm:get_total_pooled_resource_changed_for_faction(faction:name(), resource, factor)
...
cm:add_first_tick_callback(function()
    ...
    for _, key in ipairs(human_faction_keys) do
        if cm:are_pooled_resources_tracked_for_faction(key) then
            return
        end
