    ...
    if is_uicomponent(existing_button) then
        ...
        local new_button = UIComponent(
            ui_button_parent:CreateComponent(
                button_name, "ui/templates/dev_button_small.twui.xml"
            )
        )
        return new_button
    end
...
local function fix_army_with_hero(lord_char, hero_char)
    Log("fix_army_with_hero_cqi")

    -- If there is no hero_cqi, return.
    local hero_to_embed = hero_char
    if not hero_to_embed or hero_to_embed:is_null_interface() then
        Log("handle_selected_army:hero_to_embed is nil")
        return
    end

    local lord = lord_char
    ...
    if hero_to_embed:is_embedded_in_military_force() then
        Log("fix_army_with_hero_cqi:hero_to_embed:is_embedded_in_military_force() is true")
        local x, y = cm:find_valid_spawn_location_for_character_from_position(
            faction:name(), lord:logical_position_x(), lord:logical_position_y(), false, 0
        )
        Log("fix_army_with_hero_cqi: x,y is " .. x .. "," .. y)
        cm:teleport_to(cm:char_lookup_str(lord), x, y)
    end
        ...
        if og_unit.purchased_effects:num_items() > 0 then
            for j = 0, og_unit.purchased_effects:num_items() - 1 do
                ...
                elseif unit_purchasable_effect_mapping_wef[cur_key] then
                    ...
                    Log("handle_selected_army:unit_purchasable_effect not found" .. cur_key)
                end
            end
...
local function handle_selected_army(context)
    ...
    fix_army_with_hero(context:character(), hero)
end
...
core:add_listener(
    ...
    function() populate_btn(get_or_create_btn()) end, 
    true
)
...
core:add_listener(
    ...
    function(context)
        return (context:character():faction():is_human() and context:character():is_embedded_in_military_force() and
                   mount_ancillaries[context:ancillary()])
    end,
    function(context)
        Log("CharacterAncillaryGained")
        local hero = context:character()
        Log(hero)
        fix_army_with_hero(hero:embedded_in_military_force():general_character(), hero)
    end,
    true
);
